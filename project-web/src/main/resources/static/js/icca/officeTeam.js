app.controller('OfficeTeamMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster', 
	...
	$scope.queryResult = function() {
		$scope.postRequest({url: "/console/officeTeam/officeTeamList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		$scope.postRequest({url: "/console/officeTeam/delOfficeTeam?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.officeTeam_edit', {id:data.id});
	}
}]);