app.controller('ICCA_WorksEditCtrl', ['$scope', '$http', '$stateParams', '$state', '$filter', 'toaster',
	...
	if(undefined != $scope.data.id && $scope.data.id != '') {
		$scope.postRequest({url: "/console/works/worksInfo.do", data: $.param($scope.data)}, function(resp){
	...
	$scope.save = function(){
		...
		$scope.postRequest({url: "/console/works/saveWorks.do", data: $.param($scope.data)}, function(resp){
			if(resp.state == 0) {
	...
	window.updateFile = function(fileId, obj, type) {
		...
		_scope = angular.element($("#worksDivId")).scope();
