app.controller('ICCA_ContactEditCtrl', ['$scope', '$http', '$stateParams', '$state', '$filter', 'toaster', 
	...
	if(undefined != $scope.data.id && $scope.data.id != '') {
		$scope.postRequest({url: "/console/contact/contactInfo.do", data: $.param($scope.data)}, function(resp){
	...
	$scope.save = function(){
		...
		$scope.postRequest({url: "/console/contact/saveContact.do", data: $.param($scope.data)}, function(resp){
			if(resp.state == 0) {
	...
	window.updateFile = function(fileId, obj, type) {
		...
		_scope = angular.element($("#contactDivId")).scope();
