app.controller('ICCA_ArtEditCtrl', ['$scope', '$http', '$stateParams', '$state', '$filter', 'toaster', 
	...
	if(undefined != $scope.data.id && $scope.data.id != '') {
		...
		$scope.postRequest({url: "/console/art/artInfo.do", data: $.param($scope.data)}, function(resp){
	...
	$scope.save = function(){
		...
		$scope.postRequest({url: "/console/art/saveArt.do", data: $.param($scope.data)}, function(resp){
			if(resp.state == 0) {
	...
	window.updateFile = function(fileId, obj, type) {
		...
		_scope = angular.element($("#artDivId")).scope();
