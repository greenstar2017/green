app.controller('JobMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster', 
	...
	$scope.queryResult = function() {
		...
		$scope.postRequest({url: "/console/job/jobList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		...
		$scope.postRequest({url: "/console/job/delJob?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.job_edit', {id:data.id});
	}
}]);