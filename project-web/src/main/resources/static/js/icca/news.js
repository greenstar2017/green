app.controller('NewsMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster', function($rootScope,$scope, $http, $state, $stateParams, toaster) {
	...
	$scope.queryResult = function() {
		$scope.postRequest({url: "/console/news/newsList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		$scope.postRequest({url: "/console/news/delNews.do?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.news_edit', {id:data.id});
	}
}]);