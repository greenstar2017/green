app.controller('OfficeMemberMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster',
	...
	$scope.queryResult = function() {
		...
		$scope.postRequest({url: "/console/officeMember/officeMemberList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		$scope.postRequest({url: "/console/officeMember/delOfficeMember?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.officeMember_edit', {id:data.id});
	}
}]);