app.controller('ContactMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster', 
	...
	$scope.queryResult = function() {
		...
		$scope.postRequest({url: "/console/contact/contactList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		...
		$scope.postRequest({url: "/console/contact/delContact?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.contact_edit', {id:data.id});
	}
}]);