app.controller('ArtMngCtrl', ['$rootScope','$scope', '$http', '$state', '$stateParams', 'toaster', 
	...
	$scope.queryResult = function() {
		...
		$scope.postRequest({url: "/console/art/artList.do", data: $.param($scope.params)}, function(resp){
			$scope.result = resp;
	...
	$scope.goDelete = function(data) {
		...
		$scope.postRequest({url: "/console/art/delArt.do?id="+data.id, data: ''}, function(resp){
	...
	$scope.goEdit = function(data) {
		$state.go('app.art_edit', {id:data.id});
	}
}]);